...
export class SymbolNode {
  ...
  public static createSymbolTree(symbols: vscode.DocumentSymbol[]): SymbolNode {
    ...
    let properRanges =
      symbols.find((sym) => sym.range.start.line != sym.range.end.line) != null;

    SymbolNode._createSymbolTree(root, symbols);

    if (!properRanges) {
      root._range = new vscode.Range(
        new vscode.Position(0, 0),
        new vscode.Position(1e10, 0),
      ); // whole file
  ...
  private static _createSymbolTree(
    parent: SymbolNode,
...
export class ScopeFinder {
  ...
  public static async getScopeSymbolsFor(
    uri: vscode.Uri,
  ...
  private static filterScopeSymbolsRecursive(
    symbols: vscode.DocumentSymbol[],
  ): vscode.DocumentSymbol[] {
    return symbols
      .filter((sym) => ScopeSymbolKind.indexOf(sym.kind) != -1)
      .map((sym) => {
