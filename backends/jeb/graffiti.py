            ...
            while True:
                ...
                if "type" in data and data["type"] == "auth_req_v1":
                    ...
                    class AskForTokenRunnable(Runnable):
                        def run(x):
                            token = self.ctx.displayQuestionBox(
                                "Authentication",
                ...
                Display.getDefault().asyncExec(
                    GraffitiHandlePullOnUI(self.ctx, data["address"])
                )
            self.sock.close()
        except:
            traceback.print_exc(file=sys.stdout)
    ...
    def run(self):
        try:
            ...
            for dexunit in RuntimeProjectUtil.findUnitsByType(prj, IDexUnit, False):
                ...
                print(
                    "Navigation to %s failed. No matching Dex Unit found."
                    % self.address
                )
                return
            ...
            if javaunit is None:
                print(
                    "Navigation to %s failed. Couldn't decompile class %s"
                    % (self.address, clazz)
                )
                return
            ...
            if hasattr(self.ctx, "navigate"):
                ...
                if not self.ctx.openView(javaunit):
                    ...
                    return
                self.ctx.getFocusedView().getActiveFragment().setActiveAddress(
                    self.address
                )
        except:
            traceback.print_exc(file=sys.stdout)
