class NetworkController {
  ...
  handleMessage(msg) {
    if (msg.type == MSG_ADD_NODE_AND_EDGE) {
      this.tabsController.onCurrent((_, controller) => {
        ...
        const nodeId = this.addNodeAndEdge(
          controller,
          selectedNode,
          msg.node,
          msg.edge,
  ...
  addNodeAndEdge(
    ...
    isExistingToNew =
      msgEdge != null && "isExistingToNew" in msgEdge
        ? msgEdge.isExistingToNew
        : isExistingToNew;
    // 2. Check if dest node exists
    const existingDestNode =
      "address" in msgNode
        ? controller.queryNode("address", msgNode.address)
        : null;
    if (existingDestNode != null) {
      ...
      if (selectedNode != null) {
        controller.addEdge(
          {
            ...createFromTo(selectedNode.id, newNode.id, isExistingToNew),
            ...(msgEdge || {}),
          },
          null,
          shouldDraw
